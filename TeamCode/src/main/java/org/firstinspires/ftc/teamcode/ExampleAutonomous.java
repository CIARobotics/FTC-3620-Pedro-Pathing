 ...
 * It includes shooting and collecting logic derived from TeleOp.
 * Sequence: shoot after 1, collect during 4, shoot after 5, collect during 7, shoot after 9.
 * Load & Launch sequence: Load (1.5s) -> Shoot (0.5s) -> Stop (0.25s)
 * Pauses: 800ms, Extra Collection: 2500ms (wiggle).
 * Path 4 and 7 run at slower speed (0.03 power) for better collection.
 * Path 10 runs at full speed (1.0 power).
 * Path 4 and 7 are divided into 2 segments with a pause and a wiggle at the end.
 * Harvester runs during the first half of Path 5 and Path 8.
 */
@Autonomous(name = "Example Pedro Autonomous", group = "Autonomous")
@Configurable // Panels
public class ExampleAutonomous extends OpMode {
    ...
    public void init() {
        ...
        panelsTelemetry.debug("Status", "Initialized with Shooting/Collecting Logic");
    ...
    public static class Paths {
        ...
        public Paths(Follower follower) {
            ...
            // Split Path4 into 2 segments (midpoint roughly 118.348, 77.639)
            ...
            // Split Path7 into 2 segments (midpoint roughly 121.240, 53.023)
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 4: // Wait for Path2, then Pause
                if (!follower.isBusy()) {
                    timer.resetTimer();
                    setPathState(7); // Jump directly to pause before Path4 (Path3 removed)
                }
                break;
            ...
            case 8: // Wait for Path4a to finish, then pause
                ...
                break;
            case 802: // Pause 800ms then follow Path4b
                if (timer.getElapsedTimeSeconds() > 0.8) {
            ...
            case 15: // Wait for Path7a to finish, then pause
                ...
                break;
            case 1502: // Pause 800ms then follow Path7b
                if (timer.getElapsedTimeSeconds() > 0.8) {
